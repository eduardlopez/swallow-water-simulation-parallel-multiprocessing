/*
 *  shwater2d solves the two dimensional shallow water equations 
  ...
  int x, y;
          ...
          for (i = 1; i < n; i++) {                        
            ...
            const double g = 9.81;
              ...
              for (ii = 0; ii < m; ii++) {
                ffx[0][ii] = Q(1, ii, i);
                ffx[1][ii] = (pow(Q(1, ii, i), 2) / Q(0, ii, i))  + 
                  (g * pow(Q(0, ii, i), 2)) / 2.0;
                ffx[2][ii] = (Q(1, ii, i) * Q(2, ii, i)) / Q(0, ii, i);
              }
          ...
          for (i = 1; i < m; i++) {       
            ...
            const double g = 9.81;
              ...
              for (jj = 0; jj < n; jj++) {
                ffy[0][jj] = Q(2, i, jj);
                ffy[1][jj] = (Q(1, i, jj) * Q(2, i, jj)) / Q(0, i, jj);    
                ffy[2][jj] = (pow(Q(2, i, jj), 2) / Q(0, i, jj))  + 
                  (g * pow(Q(0, i, jj), 2)) / 2.0;
              }
...
int main(int argc, char **argv) {
  ...
  * To make things easier, we will use a square matrix.
  */    
  M = 100;
  N = 100;
...
if(verbose && flag) {
  ...
  */
  ...
  // Print final time and size 
  fprintf(stdout, "Time %.3f seconds, Size %d [%dx%d]\n", gettime() - stime, M*N, M, N);


  /* Save results */
  if(flag) save_vtk(Q, x, y, m, n, file);
